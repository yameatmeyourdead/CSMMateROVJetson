...
class MicroROV(Component):
    ...
    # 0x00 is device
    # 0x70 and 0x2E target in quarter-microseconds target*4 & 7F and target*4 >> 7 & 7F, respectively :)
    def Update(self):
        # grab newest copy of button presses
        presses = ROVMap.getButtonPresses()
        # if button has been pressed, update microROV functions accordingly
        if(presses.a):
            self.__clamp = not self.__clamp
        
        # Map specific servo channels to updated targets. command will only be generated if not none (set to None every update) NOTE: 0x0-0x05 servo devices
