...
def doControllerForwarding():
	...
	while True:
		try:
			...
			Logger.log("Sucessfully conected......starting controller")
			break
		except: # in case of error (such as server not running yet), retry connection in 5 seconds
			Logger.log("Connection unable to be established. Retrying in 5 seconds")
			time.sleep(5)
			continue
...
ControllerThread = threading.Thread(target=startControllerForwarding)
